...
export default function CertificatePage() {
  ...
  const handleExport = (format: 'json' | 'txt' | 'pdf') => {
    ...
    if (format === 'json') {
      ...
      const blob = new Blob([text], { type: 'text/plain' }); const url2 = URL.createObjectURL(blob); const a = document.createElement('a');
      a.href = url2; a.download = `证书证件_${dateStr}.txt`; a.click(); URL.revokeObjectURL(url2);
    } else if (format === 'pdf') {
