// 农历数据 1900-2100
const LUNAR_INFO = [
  0x04bd8,0x04ae0,0x0a570,0x054d5,0x0d260,0x0d950,0x16554,0x056a0,0x09ad0,0x055d2,
  0x04ae0,0x0a5b6,0x0a4d0,0x0d250,0x1d255,0x0b540,0x0d6a0,0x0ada2,0x095b0,0x14977,
  0x04970,0x0a4b0,0x0b4b5,0x06a50,0x06d40,0x1ab54,0x02b60,0x09570,0x052f2,0x04970,
  0x06566,0x0d4a0,0x0ea50,0x06e95,0x05ad0,0x02b60,0x186e3,0x092e0,0x1c8d7,0x0c950,
  0x0d4a0,0x1d8a6,0x0b550,0x056a0,0x1a5b4,0x025d0,0x092d0,0x0d2b2,0x0a950,0x0b557,
  0x06ca0,0x0b550,0x15355,0x04da0,0x0a5b0,0x14573,0x052b0,0x0a9a8,0x0e950,0x06aa0,
  0x0aea6,0x0ab50,0x04b60,0x0aae4,0x0a570,0x05260,0x0f263,0x0d950,0x05b57,0x056a0,
  0x096d0,0x04dd5,0x04ad0,0x0a4d0,0x0d4d4,0x0d250,0x0d558,0x0b540,0x0b6a0,0x195a6,
  0x095b0,0x049b0,0x0a974,0x0a4b0,0x0b27a,0x06a50,0x06d40,0x0af46,0x0ab60,0x09570,
  0x04af5,0x04970,0x064b0,0x074a3,0x0ea50,0x06b58,0x055c0,0x0ab60,0x096d5,0x092e0,
  0x0c960,0x0d954,0x0d4a0,0x0da50,0x07552,0x056a0,0x0abb7,0x025d0,0x092d0,0x0cab5,
  0x0a950,0x0b4a0,0x0baa4,0x0ad50,0x055d9,0x04ba0,0x0a5b0,0x15176,0x052b0,0x0a930,
  0x07954,0x06aa0,0x0ad50,0x05b52,0x04b60,0x0a6e6,0x0a4e0,0x0d260,0x0ea65,0x0d530,
  0x05aa0,0x076a3,0x096d0,0x04afb,0x04ad0,0x0a4d0,0x1d0b6,0x0d250,0x0d520,0x0dd45,
  0x0b5a0,0x056d0,0x055b2,0x049b0,0x0a577,0x0a4b0,0x0aa50,0x1b255,0x06d20,0x0ada0,
  0x14b63,
];

const STEMS = ['甲','乙','丙','丁','戊','己','庚','辛','壬','癸'];
const BRANCHES = ['子','丑','寅','卯','辰','巳','午','未','申','酉','戌','亥'];
const ZODIAC = ['鼠','牛','虎','兔','龙','蛇','马','羊','猴','鸡','狗','猪'];
const LUNAR_MONTHS = ['正','二','三','四','五','六','七','八','九','十','冬','腊'];
const LUNAR_DAYS = ['初一','初二','初三','初四','初五','初六','初七','初八','初九','初十','十一','十二','十三','十四','十五','十六','十七','十八','十九','二十','廿一','廿二','廿三','廿四','廿五','廿六','廿七','廿八','廿九','三十'];

// 公历节日
const SOLAR_FESTIVALS: Record<string, string> = {
  '01-01': '元旦', '02-14': '情人节', '03-08': '妇女节', '03-12': '植树节',
  '04-01': '愚人节', '05-01': '劳动节', '05-04': '青年节', '06-01': '儿童节',
  '07-01': '建党节', '08-01': '建军节', '09-10': '教师节', '10-01': '国庆节',
  '10-31': '万圣夜', '11-01': '万圣节', '12-24': '平安夜', '12-25': '圣诞节',
};

// 农历节日
const LUNAR_FESTIVALS: Record<string, string> = {
  '01-01': '春节', '01-15': '元宵节', '05-05': '端午节',
  '07-07': '七夕', '07-15': '中元节', '08-15': '中秋节',
  '09-09': '重阳节', '12-30': '除夕',
};

// 节气（近似日期映射，简化处理）
const SOLAR_TERMS: Record<string, string> = {
  '01-05': '小寒', '01-20': '大寒', '02-04': '立春', '02-19': '雨水',
  '03-05': '惊蛰', '03-20': '春分', '04-05': '清明', '04-20': '谷雨',
  '05-05': '立夏', '05-21': '小满', '06-05': '芒种', '06-21': '夏至',
  '07-07': '小暑', '07-22': '大暑', '08-07': '立秋', '08-23': '处暑',
  '09-07': '白露', '09-23': '秋分', '10-08': '寒露', '10-23': '霜降',
  '11-07': '立冬', '11-22': '小雪', '12-07': '大雪', '12-22': '冬至',
};

function getLunarYearInfo(year: number) {
  return LUNAR_INFO[year - 1900];
}

function getLeapMonth(year: number) {
  return getLunarYearInfo(year) & 0xf;
}

function getMonthDays(year: number, month: number) {
  return (getLunarYearInfo(year) & (0x10000 >> month)) ? 30 : 29;
}

function getYearDays(year: number) {
  let sum = 348;
  for (let i = 0x8000; i > 0x8; i >>= 1) {
    sum += (getLunarYearInfo(year) & i) ? 1 : 0;
  }
  return sum + getLeapMonthDays(year);
}

function getLeapMonthDays(year: number) {
  if (getLeapMonth(year)) {
    return (getLunarYearInfo(year) & 0x10000) ? 30 : 29;
  }
  return 0;
}

export interface LunarDate {
  month: number;
  day: number;
  isLeap: boolean;
  monthStr: string;
  dayStr: string;
  yearStr: string;
  festival: string;
  term: string;
}

export function getLunarDate(date: Date): LunarDate {
  const year = date.getFullYear();
  const month = date.getMonth() + 1;
  const day = date.getDate();

  // 公历节日
  const solarKey = `${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
  const solarFest = SOLAR_FESTIVALS[solarKey] || '';

  // 节气
  const term = SOLAR_TERMS[solarKey] || '';

  // 计算农历
  const baseDate = new Date(1900, 0, 31);
  let offset = Math.floor((date.getTime() - baseDate.getTime()) / 86400000);

  let lunarYear = 1900;
  let daysInYear = getYearDays(lunarYear);
  while (lunarYear < 2100 && offset >= daysInYear) {
    offset -= daysInYear;
    lunarYear++;
    daysInYear = getYearDays(lunarYear);
  }

  let lunarMonth = 1;
  let isLeap = false;
  const leap = getLeapMonth(lunarYear);

  while (lunarMonth <= 12 && offset > 0) {
    if (leap > 0 && lunarMonth === leap + 1 && !isLeap) {
      isLeap = true;
      lunarMonth--;
      const leapDays = getLeapMonthDays(lunarYear);
      if (offset < leapDays) break;
      offset -= leapDays;
      isLeap = false;
    } else {
      const monthDays = getMonthDays(lunarYear, lunarMonth);
      if (offset < monthDays) break;
      offset -= monthDays;
    }
    lunarMonth++;
  }

  const lunarDay = offset + 1;

  const monthStr = (isLeap ? '闰' : '') + LUNAR_MONTHS[lunarMonth - 1] + '月';
  const dayStr = LUNAR_DAYS[lunarDay - 1] || String(lunarDay);

  // 农历节日
  const lunarKey = `${String(lunarMonth).padStart(2,'0')}-${String(lunarDay).padStart(2,'0')}`;
  let festival = solarFest;
  if (LUNAR_FESTIVALS[lunarKey]) {
    festival = LUNAR_FESTIVALS[lunarKey];
  }

  // 生肖年
  const zodiacIdx = (lunarYear - 4) % 12;
  const yearStr = STEMS[(lunarYear - 4) % 10] + BRANCHES[zodiacIdx] + '年 [' + ZODIAC[zodiacIdx] + ']';

  return { month: lunarMonth, day: lunarDay, isLeap, monthStr, dayStr, festival, term, yearStr };
}

/** 判断今天是不是特殊节日，返回节日名 */
export function getTodayFestival(date: Date): string {
  const lunar = getLunarDate(date);
  return lunar.festival || lunar.term || '';
}
